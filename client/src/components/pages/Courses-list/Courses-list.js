...
class CoursesList extends Component {
  ...
  filterBySearch = (value) =>
    this.setState({
      filteredCourses: [...this.state.courses].filter((elm) =>
  ...
  filterByCategory = (option) =>
    option !== "default"
      ? this.setState({
          filteredCourses: [...this.state.filteredCourses].filter(
            (elm) => elm.category === option
          ),
        })
      : this.setState({ filteredCourses: [...this.state.courses] });

  filterByLevel = (option) =>
    option !== "default"
      ? this.setState({
          filteredCourses: [...this.state.filteredCourses].filter(
            (elm) => elm.difficultyLevel === option
          ),
        })
      : this.setState({ filteredCourses: [...this.state.courses] });
  ...
  sortBy = (option) => {
    ...
    switch (option) {
      case "Name-A":
        this.setState({
          filteredCourses: filteredCoursesCopy.sort((a, b) =>
            a.title > b.title ? 1 : -1
          ),
        });
        break;
        ...
        <section className="container-fluid courses-hero">
          <Container>
            <div className="heading">
              <h1 className="mt-5">Nuestros cursos</h1>
              <p>Ayudar a los adolescenter tener una vida sexual sana.</p>
              <SearchBar
