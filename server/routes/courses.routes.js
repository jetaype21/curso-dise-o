...
router.get("/getTeacherCourses/:id", isValidId, (req, res) => {
  Course.find({ owner: req.params.id })
    .then((response) => res.json(response))
    .catch((err) => res.status(500).json(err));
});
...
router.post(
  ...
  [
    ...
    check("difficultyLevel")
      .isIn(["TODOS LOS NIVELES", "SECONDARIA", "TECNOLOGICO", "ESCOLARES"])
  ...
  (req, res) => {
    ...
    const mainTopicsArr = whatYouWillLearn
      .split(",")
...
router.delete(
  ...
  isValidId,
  (req, res) => {
    Course.deleteMany({ owner: req.params.id })
      .then((response) => res.json(response))
      .catch((err) => res.status(500).json(err));
  }
);
